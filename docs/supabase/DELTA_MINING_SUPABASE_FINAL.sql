-- DELTA MINING OPS - migracion incremental final
-- Generado para el estado live validado el 2026-08-14.
-- No modifica ni elimina registros operativos; crea/reemplaza vistas, RPCs,
-- indices y permisos necesarios sobre las tablas tipadas ya existentes.
...
language sql stable security invoker set search_path=public as $$
  ...
  select case when l.code='RCP0039' then 'RPC0039' else l.code end,
    ...
    coalesce(l.raw_data->>'NÂ° OT',l.raw_data->>'NÂº OT',l.raw_data->>'OT',l.raw_data->>'Orden de trabajo',''),
