...
function readRop02AcceleratorSheet_(name){
  ...
  return values.map(function(row){var out={};headers.forEach(function(h,i){var v=row[i];out[h]=v instanceof Date?formatDate(v):v;});return{ok:Object.keys(out).every(function(k){return out[k]&&out[k].ok!==false;}),results:out};});
}
...
function tallerRowObject_(row){var out={};TALLER_MOV_HEADERS_.forEach(function(h,i){var v=row[i];out[h]=v instanceof Date?v.toISOString():v;});return{ok:Object.keys(out).every(function(k){return out[k]&&out[k].ok!==false;}),results:out};}
 ...
 **********************************************************************/
var DELTA_SUPABASE_SYNC_VERSION_="2026-09-09-V1";
...
function deltaTypedRow_(key,entry){
  ...
  return Object.assign(base,{codigo:String(deltaPick_(r,["Codigo","Código"])||""),descripcion:String(deltaPick_(r,["Descripcion","Descripción"])||""),precio_unitario:String(deltaNumber_(deltaPick_(r,["Precio unitario","Costo unitario","Precio unitario con IVA"]))),descripcion_adicional:String(deltaPick_(r,["Descripcion adicional","Descripción adicional"])||"")});
}
function deltaSyncTyped_(key){var s=deltaReadSourceRows_(key),rows=s.rows.map(function(x){return deltaTypedRow_(key,x);});var out=deltaSupabaseRpc_("sync_typed_dataset",{p_dataset:key,p_rows:rows});out.sheetRows=rows.length;return{ok:Object.keys(out).every(function(k){return out[k]&&out[k].ok!==false;}),results:out};}
function deltaSyncGeneric_(key){var s=deltaReadSourceRows_(key),out=deltaSupabaseRpc_("sync_generic_dataset",{p_dataset:key,p_rows:s.rows,p_source_version:getDatasetVersion_(key)});out.sheetRows=s.rows.length;return{ok:Object.keys(out).every(function(k){return out[k]&&out[k].ok!==false;}),results:out};}
function deltaSyncRaba03_(){var s=deltaReadSourceRows_("raba03"),a=deltaSupabaseRpc_("sync_generic_dataset",{p_dataset:"raba03",p_rows:s.rows,p_source_version:getDatasetVersion_("raba03")}),b=deltaSupabaseRpc_("sync_sheet_storage_dataset",{p_dataset:"abastecimiento_raba03",p_rows:s.rows,p_meta:{source:"Sheets"}});return{ok:true,generic:a,abastecimiento:b,sheetRows:s.rows.length};}
function deltaSyncRemitos_(){var s=deltaReadSourceRows_("remitos_cargados"),groups={};s.rows.forEach(function(entry){var r=entry.row_data,id=String(deltaPick_(r,["ID_REMITO","id"])||"").trim();if(!id)return;if(!groups[id])groups[id]={id:id,comprobante:deltaPick_(r,["N_REMITO","comprobante"]),fecha:deltaIsoDate_(deltaPick_(r,["FECHA_REMITO","fecha"])),origen:deltaPick_(r,["ORIGEN","origen"]),destino:deltaPick_(r,["DESTINO","destino"]),proyecto:deltaPick_(r,["PROYECTO","proyecto"]),observaciones:deltaPick_(r,["OBSERVACIONES","observaciones"]),usuarioCarga:deltaPick_(r,["USUARIO_CARGA","usuarioCarga"]),fechaCargaApp:deltaPick_(r,["FECHA_CARGA_APP","fechaCargaApp"]),items:[]};groups[id].items.push({source_row:entry.source_row,codigo:deltaPick_(r,["CODIGO_ARTICULO","codigo"]),descripcion:deltaPick_(r,["DESCRIPCION","descripcion"]),cantidad:String(deltaNumber_(deltaPick_(r,["CANTIDAD_ENVIADA","cantidad"])))});});var generic=deltaSupabaseRpc_("sync_generic_dataset",{p_dataset:"remitos_cargados",p_rows:s.rows,p_source_version:getDatasetVersion_("remitos_cargados")}),storage=deltaSupabaseRpc_("sync_sheet_storage_dataset",{p_dataset:"abastecimiento_remitos",p_rows:Object.keys(groups).map(function(k){return groups[k];}),p_meta:{source:"Sheets"}});return{ok:true,generic:generic,abastecimiento:storage,sheetRows:s.rows.length};}
...
function estadoSincronizacionSupabase(){var p=PropertiesService.getScriptProperties(),triggers=ScriptApp.getProjectTriggers().filter(function(t){return t.getHandlerFunction()==="sincronizarDeltaConSupabaseCada5Min";}).map(function(t){return{handler:t.getHandlerFunction(),event:String(t.getEventType())};});return{ok:true,version:DELTA_SUPABASE_SYNC_VERSION_,configured:{url:!!p.getProperty("DELTA_SUPABASE_URL"),serviceRoleKey:!!p.getProperty("DELTA_SUPABASE_SERVICE_ROLE_KEY")},triggers:triggers,lastSync:p.getProperty("DELTA_SUPABASE_LAST_SYNC")||null};}
