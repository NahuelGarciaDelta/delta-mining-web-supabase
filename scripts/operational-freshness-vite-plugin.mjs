export function operationalFreshnessVitePlugin(){
  return {
    ...
    transform(code,id){
      ...
      const resultsMarker='      const results=await Promise.allSettled(toCheck.map(key=>fetchOneSource(key,{force,serverVersions,cacheRecords})));';
      ...
      const resultsIndex=versionsIndex<0?-1:s.indexOf(resultsMarker,versionsIndex);
      const endMarker='        setLastUpdate(new Date());\n      }';
      const endIndex=resultsIndex<0?-1:s.indexOf(endMarker,resultsIndex);
      if(versionsIndex<0||resultsIndex<0||endIndex<0)throw new Error('operationalFreshnessVitePlugin: no se pudo delimitar la carga operativa');
      ...
      s=s.slice(0,versionsIndex)+progressive+s.slice(endIndex+endMarker.length);
