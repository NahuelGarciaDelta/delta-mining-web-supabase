...
function fullOperationalWindowParams_(params={}){const days=Math.max(1,Number(params.days)||7);const hasExplicitRange=Boolean(params.desde||params.hasta);if(days>=45&&!hasExplicitRange){const {days:_days,snapshot:_snapshot,...rest}=params;return{...rest,limit:"all",sortBy:"fecha",sortDirection:"asc"};}const endIso=String(params.hasta||"").slice(0,10);const end=endIso?new Date(`${endIso}T12:00:00`):new Date();const start=new Date(end);start.setDate(start.getDate()-(days-1));const ymd=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;return{...params,desde:params.desde||ymd(start),hasta:params.hasta||ymd(end),limit:"all",sortBy:"fecha",sortDirection:"asc"};}
...
// PM y otros consumidores operativos necesitan el universo ROP02 completo del período.
// El snapshot optimizado histórico excluía vehículos en algunos caminos de consulta
// (camionetas/camiones), por eso aquí usamos siempre el dataset ROP02 completo del rango.
export const getRop02OperationalSnapshot=params=>getDataset("rop02",fullOperationalWindowParams_(params||{}));
