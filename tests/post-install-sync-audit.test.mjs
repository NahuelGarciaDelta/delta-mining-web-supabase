...
const scriptPath = fileURLToPath(new URL('./Delta_Backend_Supabase_FINAL_2026-09-09-POST-AUDIT.gs', import.meta.url));

test('post-install Apps Script keeps a verifiable audit and covers pending datasets', async () => {
  const source = await readFile(scriptPath, 'utf8');
  assert.match(source, /DELTA_SUPABASE_SYNC_VERSION_\s*=\s*"2026-09-09-POST-AUDIT-V1"/);
